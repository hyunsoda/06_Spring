...
public class LoggingAspect {
	...
	public void beforeController(JoinPoint jp) {
		...
		HttpServletRequest req = 
				((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest();
						// RequestContextHolder 현재 쓰레드의 요청을 얻을 수 있는 helper 클래스
						// currentRequestAttributes() : helper클래스 속성을 가져옴
						// (ServletRequestAttributes) : RequestAttribute라는 인터페이스 구현체
		// ==  Controller가 아닐 때 HttpServletRequest 얻어오는 법
		...
		// 로그인 상태인 경우
		if(req.getSession().getAttribute("loginMember") != null) { // req를 이용해서 session 속성 가져오기
	...
	public Object aroundServiceImpl(ProceedingJoinPoint pjp) throws Throwable {
		...
		Object obj = pjp.proceed(); // 전 / 후를 나누는 기준절
