...
@Slf4j
@Component  // Bean 등록 
@PropertySource("classpath:/config.properties")  // config.properties를 참조하겠다.
@RequiredArgsConstructor
public class ImageDeleteScheduling { // 사용하려면 bean으로 등록되어있어야 한다

	private final BoardService service;
	
	// 회원 프로필 이미지 파일 저장 경로
	@Value("${my.profile.folder-path}")   // @Value : org.springframework.beans.factory.annotation.Value;
	...
	// @Scheduled(cron = "0,15,30,45 * * * * *") // 시계 초 단위가 0,15,30,45 경우 수행  (,로 나열 가능)
	// @Scheduled(cron = "0 0 * * * *")  // 0초 0분 모든 시 => 정시마다 수행
	// @Scheduled(cron = "0 0 0 * * *") // 자정
	// @Scheduled(cron = "0 0 12 * * *") // 정오
	 @Scheduled(cron = "0 0 0 1 * *") // 매달 1일
	
	//@Scheduled(cron = "0,30 * * * * *") // 시계 초 단위가 0초, 30초인 경우 수행 (테스트용)
	public void scheduling() { // 매개변수 사용 불가
		log.info("스케쥴러 동작!");
		...
		File[]  boardArr = boardFolder.listFiles(); // 파일 배열 형태로 넘어옴
		...
		// 배열 내용 복사 ( 깊은 복사 )
		// System.arraycopy(복사할 배열 , 몇 번 인덱스부터 복사할 지 , 새로운 배열 , 새로운 배열의 몇 번 부터 넣을 지 인덱스 , 복사를 어디까지 할 건 지 )
		...
		// 배열 -> List 변환 (다루기 쉽도록)
		List<File> serverImageList = Arrays.asList(imageArr);  // List화 해줌
		...
		if(!serverImageList.isEmpty()) { // 서버에 이미지가 있을 경우
			...
			for(File serverImage : serverImageList) {
				...
				// 존재하지 않으면 -1 반환하는 메서드
				if(dbImageList.indexOf(serverImage.getName())== -1) { // DB에 서버에 있는 이미지가 없다면
