...
public class MyPageServiceImpl implements MyPageService{
	...
	public String fileUpload1(MultipartFile uploadFile) throws Exception{
		...
		// 		원하는 경로에 전송(서버 어떤 폴더에 저장할지 지정)     	// 실제 원하는 경로로 보냄
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws IOException{
		 ...
		 * 3) 만약에 파일 저장 실패 시
		 * 		-> 예외 발생
		 * 		-> @Transactonal을 이용해서 rollback 수행
		 * 
		 * */
		...
		String fileRename = Utility.fileRename(uploadFile.getOriginalFilename());  // static은 클래스명.메서드명()
		
		// Builder 패턴을 이용해서 uploadFile 객체 생성
		// 장점 1) 반복되는 참조변수명, set 구문 생략
		// 장점 2) methood chaining을 이용해서 한 줄로 작성 가능
...
//		uf.setFileRename(fileRename);
		...
		// @Transactional은 RuntimeException(Unchecked Exception 대표)만 처리
		// -> rollbackFor 속성 이용해서
		// 롤백할 예외 범위를 수정
	...
	public int fileUpload3(List<MultipartFile> aaaList, List<MultipartFile> bbbList, int memberNo) throws Exception {
		...
		for(MultipartFile file : aaaList) {
			...
			// -> 파일 하나 업로드 + DB에 INSERT
			result1 += fileUpload2(file,memberNo);
			
			
		}
		
		// 2. bbbList 처리    처리 방법 동일
		int result2 = 0;
