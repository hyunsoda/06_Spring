...
// 전달 받은 숫자가 10 미만인 경우 (한 자리) 앞에 0 붙여서 반환
...
checkAuthKeyBtn.addEventListener("click",() => {
    ...
    .then(result => {
        ...
        if(result == 0) { // result가 .text()로 String타입으로 돌아오기 때문 값만 비교
...
signUpForm.addEventListener("submit", e=> {
    ...
    for(let key in checkObj){ // checkObj 요소의 key 값을 순서대로 꺼내옴
        ...
        if( !checkObj[key] ){ // false인 경우 (유효하지 않음)  key에 맞는 value가 나옴
