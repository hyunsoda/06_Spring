...
if(updateInfo != null){
    ...
    updateInfo.addEventListener("submit", e => {
        ...
        // 입력을 안하면 전부 안해야되고
        // 입력을 하면 전부 해야된다
        const addr0 = memberAddress[0].value.trim().length == 0; // true나 false가 들어옴
        const addr1 = memberAddress[1].value.trim().length == 0; // true나 false가 들어옴
        const addr2 = memberAddress[2].value.trim().length == 0; // true나 false가 들어옴

        // 모두 true인 경우만 true 저장
        const result1 = addr0 && addr1 && addr2;  // 셋 다 true인 경우 result1이 true =>> 아무것도 입력 X
...
if(secession != null) {
    ...
    secession.addEventListener("submit", e => {
        ...
        // 약관 동의 체크 확인
        // checkbox 또는 radio  checked 속성 이용
...
// 1) 프로필 이미지 수정에 사용할 요소 모두 얻어오기
 ...
 const changeImageFn = e => {
   ...
   const file = e.target.files[0];
          // 올릴 파일에 직접 접근하려면 e.target.files의 0번 index로 접근해야 한다!
