...
public class ParameterController {
	 ...
	 * 				true이면 필수적으로 요청받을 때 publisher라는 키를 가진 애가 html에 있어야 함
	 * 				넘어올 때 꼭 없어도 되면 required=false라고 하면 됨
	 * 				value랑 같이 써줘야 함
	 ...
	 * */
	@PostMapping("test2")
	public String paramTest2(@RequestParam("title") String title, // ***기본값으로 사용하면 required=true라서 반드시 넘어올 값이 있어야 함***
				@RequestParam("writer") String writer,
				@RequestParam("price") int price,  // 자동 형변환
				@RequestParam(value="publisher", required=false, defaultValue="ABC출판사") String publisher   // 요청받을 때 파라미터값이 꼭 있어야 한다 에러남 or 속성 추가 작성해야함
			) {
	...
	public String paramTest3(
		...
		log.debug("colorArr : " + Arrays.toString(colorArr));   // 배열 요소 다 찍어볼 때 사용
		...
		// RequestParam에서 Map을 찍으면 여러 값이어도 하나만 나온다
		// 		=> 겹치지 않는 name값들을 한 번에 가지고 올 때 좋음
	...
	// *** @ModelAttribute 어노테이션은 생략이  가능한다 ***
	
	@PostMapping("test4")
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
												// -> 커맨드 객체
		// setter로 inputMember에 자동으로 세팅해준다
